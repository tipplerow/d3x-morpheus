...
class XDataFrameColumn<R,C> extends XDataFrameVector<C,R,R,C,DataFrameColumn<R,C>> implements DataFrameColumn.Cursor<R,C> {
    ...
    public final DataFrameColumn<R,C> forEachValue(Consumer<DataFrameValue<R,C>> consumer) {
        ...
        if (rowCount > 0) {
            ...
            final DataFrameCursor<R,C> cursor = frame.cursor().atOrdinals(0, colOrdinal);
    ...
    public boolean isNull(R key) {
        var rowCoord = data.rowCoordinateOrFail(key);
        var colCoord = data.colCoordinateOrFail(colKey);
    ...
    @Override
    public final Cursor<R,C> add(C colKey, Class<?> dataType) {
        if (frame.cols().contains(colKey)) {
            return atKey(colKey);
        } else {
            this.frame.cols().add(colKey, dataType);
            return atKey(colKey);
        }
    }
    ...
    public final boolean getBoolean(R key) {
        final int rowIndex = data.rowCoordinateOrFail(key);
