...
public interface Index<K> extends Iterable<K>, Cloneable, Serializable {
    ...
    /**
     * Returns the ordinal for the canonical coordinate specified
     * @param coordinate    the coordinate to find the respective ordinal
     * @return              the ordinal for canonical coordinate
     */
    int getOrdinalAt(int coordinate);

    /**
     * Returns the canonical coordinate for the key specified, or -1
     * @param key   the key for which to find the canonical coordinate
     * @return      the canonical coordinate for key, or -1 if no match
     */
    int getCoordinate(K key);

    /**
     * Returns the canonical position for the ordinal specified, or -1
     * @param ordinal   the ordinal of the key for which to find the canonical position
     * @return          the canonical position for the ordinal, or -1 if no match
     * @throws IndexException   if index is out of bounds
     */
    int getCoordinateAt(int ordinal);
