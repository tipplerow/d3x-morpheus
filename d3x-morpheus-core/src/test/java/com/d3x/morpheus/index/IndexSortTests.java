...
public class IndexSortTests {
    ...
    public <T extends Comparable> void testSort(Array<T> array) {
        ...
        for (int i = 0; i < array.length(); ++i) {
            final T key = array.getValue(i);
            Assert.assertEquals(index.getCoordinate(key), i, "The index matches expected value");
            Assert.assertEquals(index.getOrdinal(key), i, "The index matches expected value");
        }
    }
    ...
    public <T extends Comparable> void testSortFilter(Array<T> array) {
        ...
        for (int i = 0; i < filter.size(); ++i) {
            ...
            final int actual = filter.getCoordinate(key);
            final int expected = index.getCoordinate(key);
