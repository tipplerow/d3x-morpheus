...
public class IndexFilterTests {
    ...
    public void testLocalDateFilter() {
        ...
        for (int i=0; i<mondays.size(); ++i) {
            ...
            Assert.assertEquals(mondays.getCoordinate(date), index.getCoordinate(date), "Indexes match");
            Assert.assertEquals(mondays.getCoordinate(date), mondays.getCoordinateAt(i));
        }
    }
}
