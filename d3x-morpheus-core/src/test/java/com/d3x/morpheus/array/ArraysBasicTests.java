...
public class ArraysBasicTests {
    ...
    public void testLocalDateTimeArray(boolean sparse) {
        ...
        for (int i=0; i<values.length; ++i) {
            final int minCount = (int)(Math.random() * 100);
            final LocalDateTime value = LocalDateTime.now().truncatedTo(ChronoUnit.MILLIS).minusMinutes(minCount);
            values[i] = value;
            array.setValue(i, value);
        }
        for (int i=0; i<values.length; ++i) {
    ...
    public void testZonedDateTimeArray(boolean sparse) {
        ...
        for (int i=0; i<values.length; ++i) {
            ...
            final ZonedDateTime value = ZonedDateTime.now(zoneId).truncatedTo(ChronoUnit.MILLIS).minusMinutes(minCount);
            values[i] = value;
            array.setValue(i, value);
        }
        for (int i=0; i<values.length; ++i) {
    ...
    public <T> void testFill(Class<T> type, ArrayStyle style) {
        ...
        if (array.typeCode() == ArrayType.BOOLEAN) {
            ...
            T value = (T)LocalDateTime.now().truncatedTo(ChronoUnit.MILLIS);
            array.fill(value);
            for (int i=0; i<array.length(); ++i) {
                Assert.assertEquals(array.getValue(i), value, "Values match at " + i);
            }
        } else if (array.typeCode() == ArrayType.ZONED_DATETIME) {
            T value = (T)ZonedDateTime.now().truncatedTo(ChronoUnit.MILLIS);
            array.fill(value);
            for (int i=0; i<array.length(); ++i) {
                Assert.assertEquals(array.getValue(i), value, "Values match at " + i);
            }
        } else if (array.typeCode() == ArrayType.OBJECT) {
            T value = (T)Double.valueOf(7d);
            array.fill(value);
            for (int i=0; i<array.length(); ++i) {
                Assert.assertEquals(array.getValue(i), value, "Values match at " + i);
            }
    ...
    public <T> void testFirstAndLast(Class<T> type, ArrayStyle style) {
        ...
        if (arrayType == ArrayType.BOOLEAN) {
            ...
            for (int i=0; i<values.length; ++i) values[i] = LocalDateTime.now().truncatedTo(ChronoUnit.MILLIS).plusSeconds(i);
            ...
            for (int i=0; i<values.length; ++i) values[i] = ZonedDateTime.now().truncatedTo(ChronoUnit.MILLIS).plusSeconds(i);
