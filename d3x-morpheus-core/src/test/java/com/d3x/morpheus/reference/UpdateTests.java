    ...
    public void testUpdateValuesOnly(Class type) {
        ...
        for (int i=0; i<frame1.rowCount(); ++i) {
            for (int j = 0; j<frame1.colCount(); ++j) {
                switch (ArrayType.of(type)) {
                    case BOOLEAN:   Assert.assertEquals(frame1.getBooleanAt(i,j), frame2.getBooleanAt(i,j));      break;
                    case INTEGER:   Assert.assertEquals(frame1.getIntAt(i,j), frame2.getIntAt(i,j));              break;
                    case LONG:      Assert.assertEquals(frame1.getLongAt(i,j), frame2.getLongAt(i,j));            break;
                    case DOUBLE:    Assert.assertEquals(frame1.getDoubleAt(i,j), frame2.getDoubleAt(i,j));        break;
                    case STRING:    Assert.assertEquals((Object)frame1.getValueAt(i,j), frame2.getValueAt(i,j));  break;
                    case OBJECT:    Assert.assertEquals((Object)frame1.getValueAt(i,j), frame2.getValueAt(i,j));  break;
                    default:    throw new IllegalArgumentException("Unsupported type: " + type);
                }
            }
        }
        ...
        for (int i=0; i<frame1.rowCount(); ++i) {
            for (int j = 0; j<frame1.colCount(); ++j) {
                switch (ArrayType.of(type)) {
                    ...
                    case LONG:      Assert.assertEquals(frame1.getLongAt(i,j), frame2.getLongAt(i,j));            break;
                    case DOUBLE:    Assert.assertEquals(frame1.getDoubleAt(i,j), frame2.getDoubleAt(i,j));        break;
                    case STRING:    Assert.assertEquals((String)frame1.getValueAt(i,j), frame2.getValueAt(i,j));  break;
                    case OBJECT:    Assert.assertEquals((Object)frame1.getValueAt(i,j), frame2.getValueAt(i,j));  break;
                    default:    throw new IllegalArgumentException("Unsupported type: " + type);
                }
            }
