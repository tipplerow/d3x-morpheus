...
public class DimensionTests {
    ...
    public void testIndexOf() {
        Assert.assertEquals(frame.rows().ordinal(rowKeys.getKey(2)), 2);
        Assert.assertEquals(frame.cols().ordinal(colKeys.getKey(4)), 4);
    }


    @Test(expectedExceptions = { DataFrameException.class })
    public void testIndexOfFailsForRow() {
        Assert.assertEquals(frame.rows().ordinalOrFail(LocalDate.now()), -1);
    }


    @Test(expectedExceptions = { DataFrameException.class })
    public void testIndexOfFailsForColumn() {
        Assert.assertEquals(frame.cols().ordinalOrFail("C88"), -1);
    }
    ...
    public void testRowKeyMapping() {
        ...
        rowKeys.forEach(date -> colKeys.forEach(column -> {
            final double value1 = frame.getDouble(date, column);
            final double value2 = result.getDouble(date.toString(), column);
    ...
    public void testColKeyMapping() {
        ...
        rowKeys.forEach(date -> colKeys.forEach(column -> {
            final double value1 = frame.getDouble(date, column);
            final double value2 = result.getDouble(date, "Column:" + column);
            Assert.assertEquals(value1, value2, "Values match for " + date + " and " + column);
        }));
    }
    ...
    public void testFindFirstRow() {
        ...
        Stream.of(frame1, frame2, frame3, frame4).forEach(frame -> {
            ...
            final Optional<DataFrameRow<String, String>> rowOption = frame.rows().first(row -> row.getDoubleAt(3) == 1000d * 3);
    ...
    public void testFindFirstColumn() throws IOException {
        ...
        Stream.of(frame1, frame2, frame3, frame4).forEach(frame -> {
            ...
            final Optional<DataFrameColumn<String, String>> colOption = frame.cols().first(column -> column.getDoubleAt(3) == 1000d * 3);
    ...
    public void testFindLastRow() {
        ...
        Stream.of(frame1, frame2).forEach(frame -> {
            ...
            frame.row("R7").applyDoubles(v -> 1000d * v.colOrdinal());
            frame.row("R9").applyDoubles(v -> 1000d * v.colOrdinal());
            final Optional<DataFrameRow<String, String>> rowOption = frame.rows().last(row -> row.getDoubleAt(3) == 1000d * 3);
            Assert.assertTrue(rowOption.isPresent(), "A row was found");
    ...
    public void testFindLastColumn() throws IOException {
        ...
        Stream.of(frame1, frame2).forEach(frame -> {
            ...
            frame.col("C15").applyDoubles(v -> 1000d * v.rowOrdinal());
            frame.col("C22").applyDoubles(v -> 1000d * v.rowOrdinal());
            final Optional<DataFrameColumn<String, String>> colOption = frame.cols().last(column -> column.getDoubleAt(3) == 1000d * 3);
            Assert.assertTrue(colOption.isPresent(), "A column was found");
