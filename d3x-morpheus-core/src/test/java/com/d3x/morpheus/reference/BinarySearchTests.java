...
public class BinarySearchTests {
    ...
    public void testRowSearchFirstAndLast() {
        ...
        final double firstActual = frame.rowAt(0).getDoubleAt(firstMatch.get().colOrdinal());
        Assert.assertEquals(frame.cols().ordinal(firstMatch.get().colKey()), 0, "Index matches");
        ...
        final double lastActual = frame.rowAt(0).getDoubleAt(lastMatch.get().colOrdinal());
        Assert.assertEquals(frame.cols().ordinal(lastMatch.get().colKey()), values.length()-1, "Index matches");
    ...
    public void testColumnSearchFirstAndLast() {
        ...
        Assert.assertTrue(firstMatch.isPresent());
        final double firstActual = frame.colAt(0).getDoubleAt(firstMatch.get().rowOrdinal());
        Assert.assertTrue(firstMatch.isPresent());
        Assert.assertEquals(frame.rows().ordinal(firstMatch.get().rowKey()), 0, "Index matches");
        Assert.assertEquals(firstActual, first, "Values match");

        final double last = values.getDouble(values.length()-1);
        final Optional<DataFrameValue<Integer,String>> lastMatch = frame.colAt(0).binarySearch(last);
        Assert.assertTrue(lastMatch.isPresent());
        final double lastActual = frame.colAt(0).getDoubleAt(lastMatch.get().rowOrdinal());
        Assert.assertTrue(lastMatch.isPresent());
        Assert.assertEquals(frame.rows().ordinal(lastMatch.get().rowKey()), values.length()-1, "Index matches");
        Assert.assertEquals(lastActual, last, "Values match");
    }
