        ...
        if (type == boolean.class) {
            source.cols().forEach(column -> {
                ...
                for (int i = 0; i < source.rowCount(); ++i) {
                    targetColumn.setBooleanAt(i, column.getBooleanAt(i));
                }
            });
            ...
            source.cols().forEach(column -> {
                final DataFrameColumn targetColumn = target.col(column.key());
                for (int i = 0; i < source.rowCount(); ++i) {
                    targetColumn.setIntAt(i, column.getIntAt(i));
                }
            });
            ...
            source.cols().forEach(column -> {
                final DataFrameColumn targetColumn = target.col(column.key());
                for (int i = 0; i < source.rowCount(); ++i) {
                    targetColumn.setLongAt(i, column.getLongAt(i));
                }
            });
            ...
            source.cols().forEach(column -> {
                final DataFrameColumn targetColumn = target.col(column.key());
                for (int i = 0; i < source.rowCount(); ++i) {
                    targetColumn.setDoubleAt(i, column.getDoubleAt(i));
                }
            });
            ...
            source.cols().forEach(column -> {
                final DataFrameColumn targetColumn = target.col(column.key());
                for (int i = 0; i < source.rowCount(); ++i) {
                    targetColumn.setValueAt(i, column.getValueAt(i));
                }
            });
        } else {
