...
public class StreamTests {
    ...
    public void testForEach() {
        ...
        frame.forEachValue(value -> {
            ...
            final int rowIndex = frame.rows().ordinal(row);
            final int colIndex = frame.cols().ordinal(column);
            final double iteratorValue = value.getDouble();
            final double expectedValue = frame.getDoubleAt(rowIndex, colIndex);
    ...
    public void testIntStream() {
        ...
        for (int colIndex = 0; colIndex<frame.colCount(); ++colIndex) {
            for (int rowIndex=0; rowIndex<frame.rowCount(); ++rowIndex) {
                ...
                final int actual= frame.getIntAt(rowIndex, colIndex);
    ...
    public void testLongStream() {
        ...
        for (int colIndex = 0; colIndex<frame.colCount(); ++colIndex) {
            for (int rowIndex=0; rowIndex<frame.rowCount(); ++rowIndex) {
                ...
                final long actual= frame.getLongAt(rowIndex, colIndex);
                Assert.assertEquals(actual, expected, "Values match at coordinates (" + rowIndex + "," + colIndex + ")");
            }
        }
    }
    ...
    public void testDoubleStream() {
        ...
        for (int colIndex = 0; colIndex<frame.colCount(); ++colIndex) {
            for (int rowIndex=0; rowIndex<frame.rowCount(); ++rowIndex) {
                ...
                final double actual= frame.getDoubleAt(rowIndex, colIndex);
    ...
    public void testValueStream() {
        ...
        for (int colIndex = 0; colIndex<frame.colCount(); ++colIndex) {
            for (int rowIndex=0; rowIndex<frame.rowCount(); ++rowIndex) {
                ...
                final Object actual= frame.getValueAt(rowIndex, colIndex);
                Assert.assertEquals(actual, expected, "Values match at coordinates (" + rowIndex + "," + colIndex + ")");
            }
        }
    }
