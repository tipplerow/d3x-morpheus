...
public class RowTests {
    ...
    public void testFirstAndLastRows() {
        ...
        frame.rows().first().get().forEachValue(v -> Assert.assertEquals(v.getDouble(), frame.getDoubleAt(v.rowOrdinal(), v.colOrdinal())));
        frame.rows().last().get().forEachValue(v -> Assert.assertEquals(v.getDouble(), frame.getDoubleAt(v.rowOrdinal(), v.colOrdinal())));
    }


    @Test(dataProvider= "args2")
    public void testForEachRow(Class type , boolean parallel) {
        final DataFrame<String,String> frame = TestDataFrames.random(type, 100000, 10);
        ...
        rows.forEach(row -> row.forEachValue(value -> {
            ...
            switch (ArrayType.of(type)) {
                ...
                case INTEGER:
                    ...
                    final int expectedInt = frame.getIntAt(rowIndex, colOrdinal);
                    assertEquals("The row indexes match", rowIndex, value.rowOrdinal());
                    assertEquals("The column indexes match", colOrdinal, value.colOrdinal());
                    assertEquals("The row keys match", frame.rows().key(rowIndex), rowKey);
                    assertEquals("The column keys match", frame.cols().key(colOrdinal), colKey);
                ...
                case LONG:
                    ...
                    final long expectedLong = frame.getLongAt(rowIndex, colOrdinal);
                    assertEquals("The row indexes match", rowIndex, value.rowOrdinal());
                    assertEquals("The column indexes match", colOrdinal, value.colOrdinal());
                    assertEquals("The row keys match", frame.rows().key(rowIndex), rowKey);
                    assertEquals("The column keys match", frame.cols().key(colOrdinal), colKey);
                ...
                case DOUBLE:
                    ...
                    final double expectedDouble = frame.getDoubleAt(rowIndex, colOrdinal);
                    assertEquals("The row indexes match", rowIndex, value.rowOrdinal());
                    assertEquals("The column indexes match", colOrdinal, value.colOrdinal());
                    assertEquals("The row keys match", frame.rows().key(rowIndex), rowKey);
                    assertEquals("The column keys match", frame.cols().key(colOrdinal), colKey);
                ...
                case STRING:
                    ...
                    final Object expectedString = frame.getValueAt(rowIndex, colOrdinal);
                    assertEquals("The row indexes match", rowIndex, value.rowOrdinal());
                    assertEquals("The column indexes match", colOrdinal, value.colOrdinal());
                    assertEquals("The row keys match", frame.rows().key(rowIndex), rowKey);
                    assertEquals("The column keys match", frame.cols().key(colOrdinal), colKey);
                ...
                case OBJECT:
                    ...
                    final Object expectedValue = frame.getValueAt(rowIndex, colOrdinal);
                    assertEquals("The row indexes match", rowIndex, value.rowOrdinal());
                    assertEquals("The column indexes match", colOrdinal, value.colOrdinal());
                    assertEquals("The row keys match", frame.rows().key(rowIndex), rowKey);
                    assertEquals("The column keys match", frame.cols().key(colOrdinal), colKey);
    ...
    public void testIntStream()  {
        ...
        frame.rows().forEach(row -> {
            ...
            for (int i = 0; i < row.size(); ++i) {
                ...
                final int expected = row.getIntAt(i);
    ...
    public void testLongStream() {
        ...
        frame.rows().forEach(row -> {
            ...
            for (int i=0; i<row.size(); ++i) {
                ...
                final long expected = row.getLongAt(i);
                Assert.assertEquals(actual, expected, "Row values match at coordinates (" + rowKey + "," + colKey + ")");
            }
        });
    }
    ...
    public void testDoubleStream() {
        ...
        frame.rows().forEach(row -> {
            ...
            for (int i=0; i<row.size(); ++i) {
                ...
                final double expected = row.getDoubleAt(i);
                Assert.assertEquals(actual, expected, "Row values match at coordinates (" + rowKey + "," + colKey + ")");
            }
        });
    }
    ...
    public void testValueStream() {
        ...
        frame.rows().forEach(row -> {
            ...
            for (int i=0; i<row.size(); ++i) {
                ...
                final Object expected = row.getValueAt(i);
                Assert.assertEquals(actual, expected, "Row values match at coordinates (" + rowKey + "," + colKey + ")");
            }
        });
    }
    ...
    @Test()
    public void testRowIteratorWithPredicate() {
    ...
    public void testRowToDataFrame(Class type) throws Exception {
        ...
        if (type == boolean.class) {
            row.forEachValue(v -> Assert.assertEquals(v.getBoolean(), frame.getBoolean(v.rowKey(), v.colKey())));
            final DataFrame<String,String> columnFrame = row.toDataFrame();
            row.forEachValue(v -> Assert.assertEquals(v.getBoolean(), columnFrame.getBoolean(v.rowKey(), v.colKey())));
        } else if (type == int.class) {
            row.forEachValue(v -> Assert.assertEquals(v.getInt(), frame.getInt(v.rowKey(), v.colKey())));
            final DataFrame<String,String> columnFrame = row.toDataFrame();
            row.forEachValue(v -> Assert.assertEquals(v.getInt(), columnFrame.getInt(v.rowKey(), v.colKey())));
        } else if (type == long.class) {
            row.forEachValue(v -> Assert.assertEquals(v.getLong(), frame.getLong(v.rowKey(), v.colKey())));
            final DataFrame<String,String> columnFrame = row.toDataFrame();
            row.forEachValue(v -> Assert.assertEquals(v.getLong(), columnFrame.getLong(v.rowKey(), v.colKey())));
        } else if (type == double.class) {
            row.forEachValue(v -> Assert.assertEquals(v.getDouble(), frame.getDouble(v.rowKey(), v.colKey())));
            final DataFrame<String,String> columnFrame = row.toDataFrame();
            row.forEachValue(v -> Assert.assertEquals(v.getDouble(), columnFrame.getDouble(v.rowKey(), v.colKey())));
        } else if (type == String.class) {
            row.forEachValue(v -> Assert.assertEquals(v.getValue().toString(), frame.getValue(v.rowKey(), v.colKey()).toString()));
            final DataFrame<String,String> columnFrame = row.toDataFrame();
            row.forEachValue(v -> Assert.assertEquals(v.getValue().toString(), columnFrame.getValue(v.rowKey(), v.colKey()).toString()));
        } else {
    ...
    public void testRowMapping1(Class type) {
        ...
        for (int rowOrdinal=0; rowOrdinal<frame.rowCount(); ++rowOrdinal) {
            for (int colOrdinal = 0; colOrdinal<frame.colCount(); ++colOrdinal) {
                if (type == boolean.class)  Assert.assertEquals(frame.getBooleanAt(rowOrdinal, colOrdinal), rowOrdinal % 2 == 0, "Match at " + rowOrdinal + ", " + colOrdinal);
                if (type == int.class)      Assert.assertEquals(frame.getIntAt(rowOrdinal, colOrdinal), rowOrdinal * 10, "Match at " + rowOrdinal + ", " + colOrdinal);
                if (type == long.class)     Assert.assertEquals(frame.getLongAt(rowOrdinal, colOrdinal), rowOrdinal * 10L, "Match at " + rowOrdinal + ", " + colOrdinal);
                if (type == double.class)   Assert.assertEquals(frame.getDoubleAt(rowOrdinal, colOrdinal), rowOrdinal * 10d, "Match at " + rowOrdinal + ", " + colOrdinal);
    ...
    public void testRowMapping2(Class type) {
        ...
        if (type == boolean.class) {
            target.rows().keys().forEach(rowKey -> {
                target.row(rowKey).applyBooleans(v -> {
                    return source.getBoolean(rowKey, v.colKey());
                });
            });
            DataFrameAsserts.assertEqualsByIndex(source, target);
        } else if (type == int.class) {
            target.rows().keys().forEach(rowKey -> {
                target.row(rowKey).applyInts(v -> {
                    return source.getInt(rowKey, v.colKey());
                });
            });
            DataFrameAsserts.assertEqualsByIndex(source, target);
        } else if (type == long.class) {
            target.rows().keys().forEach(rowKey -> {
                target.row(rowKey).applyLongs(v -> {
                    return source.getLong(rowKey, v.colKey());
                });
            });
            DataFrameAsserts.assertEqualsByIndex(source, target);
        } else if (type == double.class) {
            target.rows().keys().forEach(rowKey -> {
                target.row(rowKey).applyDoubles(v -> {
                    return source.getDouble(rowKey, v.colKey());
                });
            });
            DataFrameAsserts.assertEqualsByIndex(source, target);
        } else {
            target.rows().keys().forEach(rowKey -> {
                target.row(rowKey).applyValues(v -> {
                    return source.getValue(rowKey, v.colKey());
                });
            });
            DataFrameAsserts.assertEqualsByIndex(source, target);
        }
    ...
    public void testSelectWithParallelProcessing() {
        ...
        final Index<ZonedDateTime> timestamps = Range.of(0, 1000000).map(start::plusMinutes).toIndex(ZonedDateTime.class);
    ...
    public void testStreamOfRows() {
        ...
        frame.rows().stream().forEach(row -> {
            ...
            for (int i=0; i<frame.cols().count(); ++i) {
                final double v1 = row.getDoubleAt(i);
