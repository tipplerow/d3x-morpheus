...
public class EquityReturns {
        ...
        Stream<DataFrame<LocalDate,String>> frames = Stream.of(tickers).map(ticker -> {
            ...
            returns.rows().forEach(row -> {
                ...
                if (rowOrdinal == 0) {
                    ...
                    final double startPrice = closePrice.getDoubleAt(0,0);
                    row.applyDoubles(v -> {
                        final double currentPrice = closePrice.getDoubleAt(rowOrdinal, 0);
        ...
        Stream<DataFrame<LocalDate,String>> frames = Stream.of(tickers).map(ticker -> {
            ...
            returns.rows().forEach(row -> {
                ...
                if (rowOrdinal == 0) {
                    ...
                    row.applyDoubles(v -> {
                        final double priorPrice = closePrice.getDoubleAt(rowOrdinal-1,0);
                        final double currentPrice = closePrice.getDoubleAt(rowOrdinal, 0);
    ...
    DataFrame<LocalDate,String> computeReturns(DataFrame<LocalDate,String> prices) {
        ...
        return returns.applyDoubles(v -> {
            ...
            if (rowOrdinal == 0) {
                ...
                final double pricePrior = prices.getDoubleAt(rowOrdinal-1, colOrdinal);
                final double priceCurrent = prices.getDoubleAt(rowOrdinal, colOrdinal);
    ...
    public void simulateOutcomes() throws Exception {
        ...
        holdings.rows().forEach(row -> {
            ...
            riskReturn.rows().setDoubleAt(row.ordinal(), "Risk", risk);
            riskReturn.rows().setDoubleAt(row.ordinal(), "Return", ret);
        });
