...
public class ArrayPerf2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String key : callableMap.keySet()) {
            ...
            initTimes.rows().setDouble(key, 0, stats.getCallTime(StatType.MEDIAN));
            gcTimes.rows().setDouble(key, 0, stats.getGcTime(StatType.MEDIAN));
            memory.rows().setDouble(key, 0, stats.getUsedMemory(StatType.MEDIAN));
        }
