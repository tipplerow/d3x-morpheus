...
public class WormTableTest {
    ...
    public void testContains() {
        Assert.assertTrue(fixed.contains(1, "two"));
        Assert.assertFalse(fixed.contains(1, "three"));
        Assert.assertFalse(fixed.contains(2, "two"));
    }

    @Test
    public void testContainsColumn() {
        Assert.assertTrue(fixed.containsColumn("two"));
        Assert.assertFalse(fixed.containsColumn("foo"));
    }

    @Test
    public void testContainsRow() {
        Assert.assertTrue(fixed.containsRow(1));
        Assert.assertFalse(fixed.containsRow(2));
    }
    ...
    public void testGetOrAssignConcurrent() {
        var cells = new ArrayList<Table.Cell<Integer, String, String>>();
        var table = new WormTable<Integer, String, String>();

        while (cells.size() < 50)
            cells.add(Tables.immutableCell(1, "A", Integer.toString(cells.size())));

        cells.parallelStream().forEach(cell -> table.getOrAssign(
                Objects.requireNonNull(cell.getRowKey()),
                Objects.requireNonNull(cell.getColumnKey()),
                Objects.requireNonNull(cell.getValue())));

        // Assert that only one value was assigned...
