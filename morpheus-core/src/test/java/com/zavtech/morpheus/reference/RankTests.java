...
public class RankTests {
    ...
    public void testRankOfRows() {
        ...
        rankFrame.rows().forEach(rankRow -> {
            ...
            for (int i = 0; i < ranks.length; ++i) {
                final double value = frame.row(key).getDoubleAt(i);
                final double expected = ranks[i];
                final double actual = rankFrame.row(key).getDoubleAt(i);
    ...
    public void testRankOfColumns() {
        ...
        rankFrame.cols().forEach(rankColumn -> {
            ...
            final double[] ranks = ranking.rank(values);
            for (int i=0; i<ranks.length; ++i) {
                final double value = frame.col(key).getDoubleAt(i);
                final double expected = ranks[i];
                final double actual = rankFrame.col(key).getDoubleAt(i);
