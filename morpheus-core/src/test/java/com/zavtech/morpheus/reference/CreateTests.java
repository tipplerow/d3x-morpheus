...
public class CreateTests {
    ...
    public void testConstruction1() {
        ...
        frame2.col("C1").applyDoubles(v -> frame1.getDouble(v.rowKey(), "C1"));
        frame2.col("C2").applyInts(v -> frame1.getInt(v.rowKey(), "C2"));
        frame2.col("C3").applyBooleans(v -> frame1.getBoolean(v.rowKey(), "C3"));
        frame2.col("C4").applyDoubles(v -> frame1.getDouble(v.rowKey(), "C4"));
        frame2.col("C5").applyLongs(v -> frame1.getLong(v.rowKey(), "C5"));
        frame2.col("C6").applyValues(v -> frame1.getValue(v.rowKey(), "C6"));

        frame3.row("R1").applyValues(v -> frame1.getValue("R1", v.colKey()));
        frame3.row("R2").applyValues(v -> frame1.getValue("R2", v.colKey()));
        frame3.row("R3").applyValues(v -> frame1.getValue("R3", v.colKey()));
        frame3.row("R4").applyValues(v -> frame1.getValue("R4", v.colKey()));
        frame3.row("R5").applyValues(v -> frame1.getValue("R5", v.colKey()));
        frame3.row("R6").applyValues(v -> frame1.getValue("R6", v.colKey()));
        frame3.row("R7").applyValues(v -> frame1.getValue("R7", v.colKey()));
