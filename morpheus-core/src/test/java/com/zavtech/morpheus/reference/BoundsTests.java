...
public class BoundsTests {
    ...
    public <T> void testRowBounds(Class<T> type) {
        ...
        frame.rows().forEach(row -> {
            ...
            final DataFrameValue<String,String> min = row.<Comparable>min(v -> !v.isNull()).orElse(null);
            final DataFrameValue<String,String> max = row.<Comparable>max(v -> !v.isNull()).orElse(null);
    ...
    public void testColumnBounds() {
        ...
        frame.cols().forEach(column -> {
            ...
            Assert.assertTrue(bounds.isPresent(), "Bounds exist");
            final DataFrameValue<Integer,String> min = column.min().orElse(null);
            final DataFrameValue<Integer,String> max = column.max().orElse(null);
            final Comparable lower = bounds.get().lower();
            final Comparable upper = bounds.get().upper();
            Assert.assertEquals(lower, min.getValue(), "Lower bounds matches min value");
            Assert.assertEquals(upper, max.getValue(), "Upper bounds matches max value");
