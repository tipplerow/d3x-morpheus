...
public class CopyTests {
    ...
    public void testCopy(Class type) {
        ...
        switch (ArrayType.of(type)) {
            case BOOLEAN:   copy.applyBooleans(v -> !frame.getBooleanAt(v.rowOrdinal(), v.colOrdinal()));    break;
            case INTEGER:   copy.applyInts(v -> frame.getIntAt(v.rowOrdinal(), v.colOrdinal()) + 10);        break;
            case LONG:      copy.applyLongs(v -> frame.getLongAt(v.rowOrdinal(), v.colOrdinal()) + 10L);     break;
            case DOUBLE:    copy.applyDoubles(v -> Math.random() * 10d);                                        break;
            case STRING:    copy.applyValues(v -> "X" + frame.getValueAt(v.rowOrdinal(), v.colOrdinal()));   break;
            case OBJECT:    copy.applyValues(v -> "X" + frame.getValueAt(v.rowOrdinal(), v.colOrdinal()));   break;
        }
        frame.forEachValue(v -> {
            final Object v1 = v.getValue();
            final Object v2= copy.getValue(v.rowKey(), v.colKey());
            if (v1 != null && v2 != null) {
