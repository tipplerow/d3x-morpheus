...
public class CombineTests {
    ...
    public void testConcatRows() {
        ...
        frame1.forEachValue(v -> {
            ...
            final double actual = frame3.getDoubleAt(v.rowOrdinal(), v.colOrdinal());
            Assert.assertEquals(actual, expected);
        });

        for (int i=10; i<frame3.rowCount(); ++i) {
            for (int j=0; j<frame3.colCount(); ++j) {
                final double expected = frame2.getDoubleAt(i-5, j);
                final double actual = frame3.getDoubleAt(i, j);
                Assert.assertEquals(actual, expected);
            }
        }
    }
