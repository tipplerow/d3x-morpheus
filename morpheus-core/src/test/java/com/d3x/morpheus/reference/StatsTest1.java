...
public class StatsTest1 {
    ...
    public void testSign(DataFrame<String, String> frame) {
        ...
        frame.rows().keys().forEach(row -> frame.cols().keys().forEach(column -> {
            final double value = frame.getDouble(row, column);
            final int sign = result.getInt(row, column);
        ...
        frame.rows().forEach(row -> row.forEachValue(v -> {
            ...
            final double value = frame.getDouble(rowKey, columnKey);
            final int sign = result.getInt(rowKey, columnKey);
