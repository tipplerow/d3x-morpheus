...
import com.d3x.morpheus.stats.Correlation;
import com.d3x.morpheus.stats.Covariance;
import com.d3x.morpheus.stats.StatType;
import com.d3x.morpheus.stats.Statistic1;
import com.d3x.morpheus.stats.Statistic2;
    ...
    public double covariance(K key1, K key2) {
        ...
        if (rowCount == 0 || colCount == 0) {
            ...
            for (int i = 0; i < colCount; ++i) {
                final double v1 = row1.getDoubleAt(i);
                final double v2 = row2.getDoubleAt(i);
            ...
            for (int i = 0; i < rowCount; ++i) {
                final double v1 = column1.getDoubleAt(i);
                final double v2 = column2.getDoubleAt(i);
                correlation.add(v1, v2);
            }
            return correlation.getValue();
        }
    ...
    private class RowStatistics extends StatisticAction {
        ...
        public void compute() {
            ...
            if (count <= threshold) {
                ...
                final DataFrameCursor<R,StatType> cursor = target.cursor().atCol(0);
                ...
                for (int rowOrdinal = from; rowOrdinal <= to; ++rowOrdinal) {
                    ...
                    cursor.atRow(rowOrdinal).setDouble(statValue);
                }
            } else {
    ...
    private class ColumnStatistics extends StatisticAction {
        ...
        public void compute() {
            ...
            if (count <= threshold) {
                ...
                final DataFrameCursor<C,StatType> cursor = target.cursor().atCol(0);
                ...
                for (int colOrdinal = from; colOrdinal <= to; ++colOrdinal) {
                    ...
                    cursor.atRow(colOrdinal).setDouble(statValue);
                }
            } else {
                final int splitCount = (to - from) / 2;
                final int midPoint = from + splitCount;
            ...
            if (remainder <= threshold) {
                ...
                for (int colOrdinal=from; colOrdinal<=to; ++colOrdinal) {
                    ...
                    for (int rowOrdinal=1; rowOrdinal<rowCount; ++rowOrdinal) {
                        final double rawValue = source.atRow(rowOrdinal).getDouble();
                        final double emaPrior = target.atRow(rowOrdinal-1).getDouble();
                        final double emaValue = rawValue * alpha + (1d - alpha) * emaPrior;
                        target.atRow(rowOrdinal).setDouble(emaValue);
                    }
                }
            } else {
                final int splitCount = (to - from) / 2;
    ...
    private class BivariateRowStatistics extends StatisticAction {
        ...
        public void compute() {
            ...
            if (count <= threshold) {
                ...
                final DataFrameCursor<K,K> cursor = result.cursor();
