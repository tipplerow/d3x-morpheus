...
class XDataFrameContent<R,C> implements Serializable, Cloneable {
    ...
    /**
     * Returns the in-memory row index for row key
     * @param rowKey    the row key
     * @return          the in-memory row index
     * @throws DataFrameException   if no match for row key
     */
    final int rowIndex(R rowKey) throws DataFrameException {
        try {
            return rowKeys.getIndexForKey(rowKey);
        } catch (Exception ex) {
            throw new DataFrameException("DataFrame access error, no match for row key: " + rowKey, ex);
        }
    }


    /**
     * Returns the in-memory row index for row ordinal
     * @param rowOrdinal    the row ordinal
     * @return              the in-memory row index
     * @throws DataFrameException   if row ordinal out of bounds
     */
    final int rowIndexAt(int rowOrdinal) {
        try {
            return rowKeys.getIndexForOrdinal(rowOrdinal);
        } catch (Exception ex) {
    ...
    /**
     * Returns the in-memory column index for column key
     * @param colKey    the column key
     * @return          the in-memory column index
     * @throws DataFrameException   if no match for column key
     */
    final int colIndex(C colKey) throws DataFrameException {
        try {
            return colKeys.getIndexForKey(colKey);
        } catch (Exception ex) {
            throw new DataFrameException("DataFrame access error, no match for column key: " + colKey, ex);
        }
    }


    /**
     * Returns the in-memory column index for column ordinal
     * @param colOrdinal    the column ordinal
     * @return              the in-memory column index
     * @throws DataFrameException   if column ordinal out of bounds
     */
    final int colIndexAt(int colOrdinal) {
        try {
            return colKeys.getIndexForOrdinal(colOrdinal);
        } catch (Exception ex) {
    ...
    private Array<?> getRowArray(R rowKey) {
        ...
        switch (ArrayType.of(type)) {
            case BOOLEAN:           return array.applyBooleans(v -> booleanAt(rowIndex, colIndexAt(v.index())));
            case INTEGER:           return array.applyInts(v -> intAt(rowIndex, colIndexAt(v.index())));
            case LONG:              return array.applyLongs(v -> longAt(rowIndex, colIndexAt(v.index())));
            case DOUBLE:            return array.applyDoubles(v -> doubleAt(rowIndex, colIndexAt(v.index())));
            case DATE:              return array.applyLongs(v -> longAt(rowIndex, colIndexAt(v.index())));
            case INSTANT:           return array.applyLongs(v -> longAt(rowIndex, colIndexAt(v.index())));
            case LOCAL_DATE:        return array.applyLongs(v -> longAt(rowIndex, colIndexAt(v.index())));
            case LOCAL_TIME:        return array.applyLongs(v -> longAt(rowIndex, colIndexAt(v.index())));
            case LOCAL_DATETIME:    return array.applyLongs(v -> longAt(rowIndex, colIndexAt(v.index())));
            default:                return array.applyValues(v -> valueAt(rowIndex, colIndexAt(v.index())));
        }
    }
    ...
    private Array<?> getColArray(C colKey) {
        ...
        final int colIndex = colKeys.getIndexForKey(colKey);
        switch (ArrayType.of(type)) {
            case BOOLEAN:           return array.applyBooleans(v -> booleanAt(rowIndexAt(v.index()), colIndex));
            case INTEGER:           return array.applyInts(v -> intAt(rowIndexAt(v.index()), colIndex));
            case LONG:              return array.applyLongs(v -> longAt(rowIndexAt(v.index()), colIndex));
            case DOUBLE:            return array.applyDoubles(v -> doubleAt(rowIndexAt(v.index()), colIndex));
            case DATE:              return array.applyLongs(v -> longAt(rowIndexAt(v.index()), colIndex));
            case INSTANT:           return array.applyLongs(v -> longAt(rowIndexAt(v.index()), colIndex));
            case LOCAL_DATE:        return array.applyLongs(v -> longAt(rowIndexAt(v.index()), colIndex));
            case LOCAL_TIME:        return array.applyLongs(v -> longAt(rowIndexAt(v.index()), colIndex));
            case LOCAL_DATETIME:    return array.applyLongs(v -> longAt(rowIndexAt(v.index()), colIndex));
            default:                return array.applyValues(v -> valueAt(rowIndexAt(v.index()), colIndex));
        }
    }
    ...
    private XDataFrameContent<R,C> copyRowStore() {
        ...
        this.colKeys().forEach(colKey -> {
            ...
            if (type.isBoolean()) {
                for (int i=0; i<rowCount; ++i) {
                    final int rowIndex = rowIndexAt(i);
    ...
    private class Cursor implements DataFrameCursor<R,C> {
        ...
        public final DataFrameCursor<R,C> atCol(int ordinal) {
            try {
                this.colIndex = colKeys.getIndexForOrdinal(ordinal);
        ...
        public final DataFrameCursor<R,C> atRowKey(R key) {
            try {
                final int ordinal = rowKeys.getOrdinalForKey(key);
        ...
        public final DataFrameCursor<R,C> atColKey(C key) {
            try {
                final int ordinal = colKeys.getOrdinalForKey(key);
