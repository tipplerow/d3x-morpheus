...
/**
 * A Parser implementation for ZonedDateTime objects.
 *
 * <p><strong>This is open source software released under the <a href="http://www.apache.org/licenses/LICENSE-2.0">Apache 2.0 License</a></strong></p>
 *
 * @author  Xavier Witdouck
 */
class ParserOfZonedDateTime extends Parser<ZonedDateTime> {
     ...
     */
    ParserOfZonedDateTime(ToBooleanFunction<String> nullChecker, Supplier<DateTimeFormatter> format) {
        super(FunctionStyle.OBJECT, ZonedDateTime.class, nullChecker);
        this.format = format;
    }
    ...
    public final ZonedDateTime apply(String value) {
        try {
            if (getNullChecker().applyAsBoolean(value)) {
                ...
                if (formatter != null) {
                    ...
                    for (Map.Entry<Pattern,DateTimeFormatter> entry : patternMap.entrySet()) {
                        final Matcher matcher = entry.getKey().matcher(value);
                        if (matcher.reset(value).matches()) {
                            return ZonedDateTime.parse(value, entry.getValue());
                        }
                    }
            ...
            throw new FormatException("Failed to parse value into ZonedDateTime: " + value, ex);
        }
    }
}
